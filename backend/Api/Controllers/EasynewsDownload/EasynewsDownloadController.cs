...
public class EasynewsDownloadController : ControllerBase
{
    ...
    [HttpGet("nzb")]
    [HttpGet("{token}/nzb")]
    public async Task<IActionResult> DownloadNzb(string? token, CancellationToken ct)
    {
        Log.Warning("EASYNEWS DOWNLOAD: token={Token}, payload={Payload}", token, Request.Query["payload"].FirstOrDefault());
        
        // Validate token if configured
        var configuredToken = _configManager.GetEasynewsToken();
        ...
        try
        {
            var ua = _configManager.GetUserAgent();
